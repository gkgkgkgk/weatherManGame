public class sucessOrFail //extends numGenerator 
{
 ...
 public sucessOrFail(WeatherMan wm, double check, City weather, double UserInput, double getPercent) 
 {
...
raise = Math.abs(UserInput - getPercent); 
  ...
  if(UserInput > 50){ // you were wrong
    ...
    else if(!outcome && getPercent > 50)  //if doesnt happen and scientists are wrong (they day its right)
  {// raise
    if(UserInput < 50){
      raise = (1)*(getPercent-UserInput);
    }
    else if(UserInput < getPercent){ // you are closer to right, but wrong
      raise = (-1)*Math.abs(UserInput-getPercent);
    }
    else{ // you are soo wrong its pathetic
      raise = (-1)*Math.abs(UserInput-getPercent);
    }
  }

  else if(!outcome && getPercent < 50) //happens and scientists are right that is doesnt happen
   {
  if(UserInput < 50){ // you were right
      raise = (-1)*(getPercent-UserInput);
    }
    else if(getPercent < UserInput){ //they guess higher than you, ya get stuffs
      raise = (-1)*Math.abs(UserInput-getPercent);
    }
    else{ // they guess more than you and doesnt happen
      raise = (-1)*Math.abs(UserInput-getPercent);
